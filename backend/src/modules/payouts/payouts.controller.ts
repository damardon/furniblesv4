...
export class PayoutsController {
  ...
  @ApiOperation({
    ...
    description:
      'Request a payout of available balance for authenticated seller',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Check if seller is eligible for payouts and available balance',
  })
  ...
  async checkEligibility(@CurrentUser() user: AuthenticatedUser) {
    try {
      ...
      const eligibility = await this.payoutsService.checkPayoutEligibility(
        user.id,
      );
      ...
      throw new BadRequestException(
        `Failed to check eligibility: ${error.message}`,
      );
    }
  }
  ...
  async getMyPayouts(
    ...
    try {
      ...
      const result = await this.payoutsService.getSellerPayouts(
        user.id,
        filters,
      );
  ...
  async getPayoutDetails(
    ...
    try {
      ...
      const payout = await this.payoutsService.getPayoutById(
        payoutId,
        user.id,
        user.role,
      );
      ...
      throw new BadRequestException(
        `Failed to get payout details: ${error.message}`,
      );
    }
  }
  ...
  @ApiQuery({
    name: 'search',
    required: false,
    description: 'Search in descriptions and seller names',
  })
  @ApiResponse({
    status: 200,
  ...
  async getAllPayouts(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to get all payouts: ${error.message}`,
      );
    }
  }
  ...
  async updatePayout(
    ...
    try {
      ...
      const result = await this.payoutsService.updatePayout(
        payoutId,
        updateDto,
        user.id,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Execute actions like retry, cancel, approve, or reject on a payout',
  })
  @ApiParam({ name: 'payoutId', description: 'Payout ID' })
  ...
  async executePayoutAction(
    ...
    try {
      this.logger.log(
        `Admin ${user.id} executing action ${actionDto.action} on payout ${payoutId}`,
      );

      const result = await this.payoutsService.executePayoutAction(
        payoutId,
        actionDto,
        user.id,
      );
      ...
      throw new BadRequestException(
        `Failed to execute payout action: ${error.message}`,
      );
    }
  }
  ...
  @ApiOperation({
    ...
    description:
      'Get comprehensive statistics about payouts across the platform',
  })
  @ApiQuery({
    name: 'startDate',
    required: false,
  ...
  async getPayoutStatistics(
    ...
    try {
      ...
      const stats = await this.payoutsService.getPayoutStatistics({
        startDate,
        endDate,
      });
      ...
      throw new BadRequestException(
        `Failed to get payout statistics: ${error.message}`,
      );
    }
  }
}
