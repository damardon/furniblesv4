...
export class StripeWebhookService {
    ...
    try {
      ...
      switch (event.type) {
        ...
        case 'transfer.updated':
          await this.handleTransferUpdated(
            event.data.object as Stripe.Transfer,
          );
          break;

        case 'transfer.reversed':
          await this.handleTransferReversed(
            event.data.object as Stripe.Transfer,
          );
          break;
  ...
  async handleConnectWebhook(
    ...
    try {
      ...
      event = this.stripeService.verifyConnectWebhookSignature(
        payload,
        signature,
      );
    } catch (err) {
    ...
    try {
      ...
      switch (event.type) {
        ...
        case 'payout.canceled':
          await this.handlePayoutCanceled(
            event.data.object as Stripe.Payout,
            event.account,
          );
          break;
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        if (order.status !== 'PENDING') {
          this.logger.warn(
            `Order ${order.id} is not in PENDING status: ${order.status}`,
          );
          return;
        }
      ...
      if (orderId) {
        try {
          ...
          this.logger.log(
            `Split payments processing deferred for order: ${orderId}`,
          );
        } catch (splitError) {
          this.logger.error(
            `Split payment failed for order ${orderId}:`,
            splitError,
          );
          // No fallar el webhook, pero log el error
          this.logger.error(
            `Split payment failure for order ${orderId}: ${splitError.message}`,
          );
        }
      }
      ...
      this.logger.log(
        `Payment processed successfully for PI: ${paymentIntent.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Error processing payment success for PI ${paymentIntent.id}`,
        {
          error: error.message,
          stack: error.stack,
        },
  ...
  private async handleTransferCreated(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (transfer.reversed) {
        this.logger.error(
          `Transfer created but was already reversed: ${transfer.id}`,
        );
        ...
        this.logger.error(
          `Transfer failure - admin should be notified: ${transfer.id} for order ${orderId}`,
        );
      } else {
        // Transfer exitoso - log instead of notification
        this.logger.log(
          `Transfer completed successfully: ${transfer.id} for order ${orderId}, amount: ${transfer.amount / 100}`,
        );
      }
    });
  }
  ...
  private async handleTransferUpdated(
    ...
    this.logger.log(
      `Transfer updated: ${transfer.id}, destination: ${transfer.destination || 'unknown'}`,
    );
  ...
  private async handleExternalAccountCreated(
    ...
    this.logger.log(
      `Seller ${accountId} added external account ${externalAccount.id}`,
    );
  }
  ...
  private async handlePayoutPaid(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Log instead of notification
      this.logger.log(
        `Payout completed: ${payout.id}, amount: ${payout.amount / 100}, account: ${accountId}`,
      );
    });
  }
  ...
  private async handlePayoutFailed(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Log instead of notification
      this.logger.error(
        `Payout failed for account ${accountId}: ${payout.failure_message}`,
      );
    });
  }

  private async handlePayoutCanceled(
    payout: Stripe.Payout,
    accountId: string,
  ): Promise<void> {
  ...
  private async handlePaymentFailed(
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        // Log instead of notification
        this.logger.warn(
          `Payment failed for order ${order.orderNumber}: ${paymentIntent.last_payment_error?.message}`,
        );
      });
    } catch (error) {
      this.logger.error(
        `Error processing payment failure for PI ${paymentIntent.id}`,
        {
          error: error.message,
          stack: error.stack,
        },
