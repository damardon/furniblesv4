  ...
  describe('Order Notifications Flow', () => {
    it('should create buyer notification on order completion', () => {
      const mockNotification = {
        ...
        message:
          'Tu orden ORD-20241224-001 ha sido completada. Ya puedes descargar tus archivos.',
        data: {
          orderId: 'order_123',
    ...
    it('should create seller notification on sale', () => {
      const mockSellerNotification = {
        ...
        data: {
          ...
          saleAmount: 150.0,
          commission: 135.0, // 90% after 10% platform fee
        },
      };

      expect(mockSellerNotification.type).toBe('NEW_SALE');
      expect(mockSellerNotification.data.commission).toBe(135.0);
    });
  });
});
