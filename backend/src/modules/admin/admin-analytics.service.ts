...
export class AdminAnalyticsService {
  ...
  async getOrderAnalytics(
    ...
    return {
      ...
      conversionRate: orderStats._count.id
        ? (completedOrders / orderStats._count.id) * 100
        : 0,
  ...
  async getProductAnalytics(
    ...
    return {
      ...
      productsByDifficulty: Object.fromEntries(
        productsByDifficulty.map((item) => [
          item.difficulty,
          item._count.difficulty,
        ]),
      ),
    };
  }
  ...
  async getFinancialAnalytics(
    ...
    const revenueByCountry = this.aggregateRevenueByCountry(
      revenueByCountryOrders,
    );
   ...
   */
  private aggregateUsersByCountry(
    orders: Array<{ buyerId: string; billingData: any }>,
  ) {
    const usersByCountry = new Map<string, Set<string>>();

    orders.forEach((order) => {
      const country =
        this.normalizeCountry(order.billingData?.country) || 'Unknown';
    ...
    return Object.fromEntries(
      Array.from(usersByCountry.entries()).map(([country, buyers]) => [
        country,
        buyers.size,
      ]),
    );
   ...
   */
  private aggregateRevenueByCountry(
    orders: Array<{ totalAmount: number; billingData: any }>,
  ) {
    ...
    orders.forEach((order) => {
      ...
      revenueByCountry[country] =
        (revenueByCountry[country] || 0) + (order.totalAmount || 0);
    });
