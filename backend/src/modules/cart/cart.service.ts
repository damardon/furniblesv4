...
export class CartService {
  ...
  async addToCart(
    ...
    if (!user || !user.buyerProfile) {
      throw new ForbiddenException(TranslationHelper.t('cart.buyerOnly', lang));
    }
  ...
  async getCart(userId: string, lang = 'en'): Promise<CartResponseDto> {
    ...
    const validItems = cartItems.filter(
      (item) => item.product && item.product.status === ProductStatus.APPROVED,
    );
    ...
    return {
      ...
      summary: {
        subtotal,
        platformFeeRate:
          feeBreakdown.find((f) => f.type === 'PLATFORM_FEE')?.rate || 0.1,
        platformFee,
        totalAmount,
  ...
  async migrateTemporaryCart(
    ...
    for (const productId of newProductIds) {
      try {
        ...
        this.logger.warn(
          `Failed to migrate product ${productId}: ${error.message}`,
        );
      }
    }
  ...
  async syncCart(
    ...
    const newItems = dto.items.filter(
      (clientItem) => !currentProductIds.includes(clientItem.productId),
    );
